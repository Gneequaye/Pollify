...
# Pollify Database Setup Script
# Run this script to create the master schema and tables

echo "Setting up Pollify database..."

# Check if running as postgres user or with sudo
if [ "$EUID" -eq 0 ] || [ "$(whoami)" = "postgres" ]; then
    PSQL_CMD="psql pollify_db"
else
    echo "Please run this script with one of the following:"
    echo "  sudo -u postgres bash setup-database.sh"
    echo "  OR run manually: sudo -u postgres psql pollify_db < master-schema.sql"
    exit 1
fi
...
\echo 'Master schema and tables created successfully!'
\dt master.*
EOF
